...
//
  ...
  it('mock function replaces method with a mock returning successfully and allows mocked method to be called with only callback',
    function () {
      const sinonStub = sinon.stub().returns('message');
      awsMock.mock('DynamoDB', 'getItem', sinonStub);
      const db: DynamoDB = new AWS.DynamoDB();
  ...
  it(
    'mock function replaces method with a mock with implementation and allows mocked method to be called with only callback',
    function () {
...
describe('AWS.setSDK function should mock a specific AWS module', function () {
  ...
  it('Modules with multi-parameter constructors can be set for mocking', function () {
