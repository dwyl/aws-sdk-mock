...
test("AWS.mock function should mock AWS service and method on the service", function (t: Test) {
  ...
  t.test(
    "mock function replaces method with a jest mock returning successfully and allows mocked method to be called with only callback",
    function (st) {
      const jestMock = jest.fn().mockReturnValue("message");
      awsMock.mock("DynamoDB", "getItem", jestMock);
      const db: DynamoDB = new AWS.DynamoDB();
  ...
  t.test("mock function replaces method with a jest mock and rejects successfully", function (st) {
    const jestMock = jest.fn<() => Promise<string>>().mockRejectedValueOnce(new Error("something went wrong"));
    awsMock.mock("DynamoDB", "getItem", jestMock);
    const db: DynamoDB = new AWS.DynamoDB();
    db.getItem({ TableName: "", Key: {} }, function (err) {
      st.equal(err.message, "something went wrong");
  ...
  t.test(
    "mock function replaces method with a jest mock with implementation and allows mocked method to be called with only callback",
    function (st) {
      const jestMock: jest.Mock<(params: any, cb: any) => any> = jest.fn((params, cb) => cb(null, "item"));
      awsMock.mock("DynamoDB", "getItem", jestMock);
      const db: DynamoDB = new AWS.DynamoDB();
